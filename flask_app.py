    ...
    if request.method == "GET":
        ...
        if results:
            ...
            if links:
                ...
                return json.dumps(results, indent=2), 200, {'Content-Type': 'application/json; charset=utf-8','Access-Control-Allow-Origin':'*'}
        else:
...
on a.playerid = d.playerid\
 LIMIT "+lim+" OFFSET "+offs+";"
    ...
    if not results:
        print("Cannot find playerID: "+ playerId)
        return "Cannot find playerID: "+ playerId, 404, {'Content-Type': 'application/json; charset=utf-8','Access-Control-Allow-Origin':'*'}
...
inner JOIN \
	(select playerid, namefirst, namelast from people) \
    as d \
on a.playerid = d.playerid  order by CAST(a.g_all AS UNSIGNED)\
 LIMIT "+lim+" OFFSET "+offs+";"
